define("focus/date-picker/0.0.2/date-picker", [ "underscore", "$", "events", "moment", "modal", "./scroll-picker", "iscroll", "./date-picker.html" ], function(require) {
    ...
    var YEAR_START = 1900; //moment().year();
    var YEAR_END = 2030;
    ...
    DatePicker.prototype = {
        ...
        handleYearMonthPick: function() {
            ...
            // 实现循环滚动
            var $picked = this.monthPicker.$list.find('li.current'),
            	pos = parseInt($picked.data('pos')),
            	count = ($picked.siblings().length - 1) / 3;
            if(pos != 2){
            	this.monthPicker.gotoIndex(this.monthPicker.getIndex() + (pos > 2 ? -1 : 1) * count);
            }
        },
        ...
        handleDayPick: function() {
            ...
            // 实现循环滚动
            var $picked = this.dayPicker.$list.find('li.current'),
            	pos = parseInt($picked.data('pos')),
            	count = ($picked.siblings().length - 1) / 3;
            if(pos != 2){
            	this.dayPicker.gotoIndex(this.dayPicker.getIndex() + (pos > 2 ? -1 : 1) * count);
            }
        },
        // 渲染日期
 ...
 * @author geliang
 */
define("focus/date-picker/0.0.2/scroll-picker", [ "underscore", "$", "events", "iscroll" ], function(require, exports, module) {
    var _ = require("underscore");
    var $ = require("$");
    var Events = require("events");
        ...
        render: function(options) {
            ...
            for(var i=1; i<4; i++) {
	            _.each(options.data, function(item) {
	                this.$list.find("li:last").before('<li data-value="' + item[1] + '" data-pos="' + i + '">' + item[0] + "</li>");
	            }, this);
            }
        },
        getIndex: function() {
