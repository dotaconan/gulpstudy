...
define(function (require, exports, module) {
    ...
    function UISlide(element, options) {
        this.$element = $(element);
        this.options = $.extend({}, UISlide.DEFAULTS, $(element).data(), options);
        this.init();
    };
    UISlide.DEFAULTS = {
        //自动播放
    ...
    //初始化
    UISlide.prototype.init = function () {
        this.autoRun = this.$element.attr('data-UISlide-autoRun') || this.options.autoRun;
        this.interval = this.$element.attr('data-UISlide-interval') || this.options.interval;
        this.speed = this.$element.attr('data-UISlide-speed') || this.options.speed;
        this.effect = this.$element.attr('data-UISlide-effect') || this.options.effect;
        this.trigger = this.$element.attr('data-UISlide-trigger') || this.options.trigger;
    ...
    //获取当前显示对象的索引值
    UISlide.prototype.getCurrentIndex = function () {
        return this.$element.find("ul li:visible").index();
    };
    //设置当前显示对象
    UISlide.prototype.setCurrentObj = function (index) {
        switch (this.effect) {
            case 'none':
    ...
    //设置当前标记对象
    UISlide.prototype.setCurrentNavigation = function (index) {
        this.$element.find(".number span:eq(" + index + ")").addClass('current').siblings('span').removeClass('current');
    };
    //箭头事件绑定
    UISlide.prototype.arrowEvent = function () {
        var _this = this;
    ...
    //导航标记事件绑定
    UISlide.prototype.navigationEvent = function () {
        var _this = this;
        //构建DOM结构
    ...
    //自动播放函数
    UISlide.prototype.autoPlay = function (i) {
        var _this = this;
    ...
    //切换事件函数
    UISlide.prototype.switchover = function (i) {
    ...
    function Plugin(option) {
        ...
        this.each(function () {
            var $this = $(this),
                data = $this.data('hc.UISlide'),
                options = typeof option === 'object' && option;
            if (!data) {
                $this.data('hc.UISlide', (data = new UISlide($this, options)));
            }
    ...
    var old = $.fn.UISlide;
    $.fn.UISlide = Plugin;
    $.fn.UISlide.Constructor = UISlide;
    $.fn.UISlide.noConflict = function () {
        $.fn.UISlide = old;
        return this;
    };
